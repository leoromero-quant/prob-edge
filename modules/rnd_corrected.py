...
def corrected_rnd(chain, spot, valuation_date, expiry_date, r_annual=0.045,
                  q_annual=0.0, n_grid: int = 400, sigma_span: float = 5.0, **_ignore):
    """
    (chain, spot, valuation_date, expiry_date, r, q) -> (K_grid, pdf), same
    producer contract as `bl`. Risk-neutral extraction via a smooth IV smile.
    """
    ...
    # Smooth smile: quadratic in log-moneyness (level + skew + curvature).
    x_obs = np.log(K_obs / F)
    deg = 2 if K_obs.size >= 3 else 1
    coeffs = np.polyfit(x_obs, iv_obs, deg)
    ...
    # Flat-wing extrapolation: clamp log-moneyness to the observed range before
    # evaluating the quadratic (avoids the parabola blowing up in the extrapolated
    # wings); IV is held at the boundary fit beyond the quoted strikes.
    x_grid = np.clip(np.log(K_grid / F), x_lo, x_hi)
    iv_grid = np.clip(np.polyval(coeffs, x_grid), 1e-3, 5.0)
